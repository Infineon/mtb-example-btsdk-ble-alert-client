...
## Overview
The ANC snippet application shows how to initialize and use the WICED Alert Notification Client Library. ANC is implemented in the GAP peripheral role and configures the Alert Notification Server(ANS) to receive Alerts based on user requests using the ClientControl Alert GUI.
...
6. If the makefile option 'TEST_HCI_CONTROL' is not used, then application button on the WICED board can be used to enable notifications.
