# Alert Notification Client (ANC) Application

## Overview
The ANC snippet application shows how to initialize and use the AIROC&#8482; Alert Notification Client Library. ANC is implemented in the GAP peripheral role and configures the Alert Notification Server(ANS) to receive Alerts based on user requests using the ClientControl Alert GUI.

## Instructions
To demonstrate the app, follow these steps:

1. Build and download the application to the AIROC&#8482; board.
2. On application init, ANC starts advertisements. The advertisement data would include "ANC". A second device acting as the ANS server running the ANS application would scan for this name and connect itself automatically when found. Once a connection is established the ClientControl ANC UI would be enabled.
3. Once connected, to get to know the Server Supported New Alerts and Unread Alerts using New Alert and Unread Alert radio buttons, the Read Alerts button has to be clicked. Once this operation is completed successfully, ANC GUI shows the server supported New Alert and Unread Alert categories. NOTE: For now, The App is designed to support Simple Alert, SMS/MMS, and Email Alert types. The remaining Alert types will not be highlighted.
4. After ANC knows the Server Supported Alerts, ANC should configure for Alert notifications as mentioned below:
    - Irrespective of Alert type, all New/Unread Alerts should be enabled by using  the EnableNewAlerts and EnableUnreadAlerts buttons.
    - Using the Control Alerts Button the user can selectively enable/disable Alerts or "Notify immediately all the pending Alerts". This can be done based on the function selection from the drop down above the Control Alerts button. In this use case the functionality for New Alerts or Unread Alerts is based on which Radio Button is enabled.
5. Once configured for Alert Notifications as mentioned above, Alerts generated from the Server (i.e ANS) can be monitored through traces.
6. If the makefile option 'TEST_HCI_CONTROL' is not used, then application button on the AIROC&#8482; board can be used to enable notifications.
